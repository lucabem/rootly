...
---------------------
Lee la tabla Hudi `orders` y escribe el resultado de un SQL de agregación
a una tabla Iceberg `order_stats`.
...
def main() -> None:
    ...
    # ── 1. Leer tabla Hudi ─────────────────────────────────────────────────────
    print(f"[1/3] Leyendo tabla Hudi desde {HUDI_ORDERS}")
    orders = spark.read.format("hudi").load(HUDI_ORDERS)
    orders.createOrReplaceTempView("orders")
    print(f"      {orders.count()} pedidos cargados")

    # ── 2. SQL de transformación ───────────────────────────────────────────────
    print("[2/3] Ejecutando SQL de agregación...")
    result = spark.sql("""
        SELECT
            customer_id,
            status,
    ...
    print("[3/3] Escribiendo tabla Iceberg → local.poc.order_stats")
    (

...
def build_spark() -> SparkSession:
    ...
    return (
        SparkSession.builder.appName("generate_data")
        .master("local[*]")
...
def create_customers_delta(spark: SparkSession) -> None:
    ...
    df = spark.createDataFrame(
        rows, ["customer_id", "name", "email", "country", "signup_date"]
    )

    (df.write.format("delta").mode("overwrite").save(DELTA_CUSTOMERS_PATH))
